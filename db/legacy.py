...
class Sync(QObject):
    ...
    def run(self):
        try:
            ...
            self.progress.emit("Phase 2/3: Mapping primary records...")
            ...
            for r in dbf_prod:
                ...
                prod_recs.append({
                    ...
                    "is_deleted": _to_bool(r.get('T_DELETED')), "is_printed": is_printed,
            ...
            if not primary_recs and not prod_recs:
                self.finished.emit(True, "Sync Info: No records found.")
                return

            self.progress.emit("Phase 3/3: Forcing legacy mirror to PostgreSQL...")
            ...
            traceback.print_exc()
            self.finished.emit(False, f"Critical Mirror Error: {e}")
...
class SyncRM(QObject):
    ...
    def run(self):
        try:
            ...
            self.finished.emit(True, "Raw Material list mirrored.")
        except Exception as e:
